...
var pJS = function (tag_id, params) {
  ...
  pJS.fn.particle.prototype.draw = function () {
    ...
    pJS.canvas.ctx.font = '50px Arial';
    ...
    // Custom draw text
    pJS.canvas.ctx.fillText(p.data ? p.data.term : '', p.x, p.y);
  ...
  pJS.fn.interact.linkParticles = function (p1, p2) {
    ...
    if (dist <= pJS.particles.line_linked.distance) {
      ...
      /*opacity_line = Math.min(
        Math.min(
          Math.min(
            Math.max((p1.x - p1.radius) - pJS.canvas.w, 0)
            , p1.x + p1.radius
          ), 
          Math.min(
            Math.max((p2.x - p2.radius) - pJS.canvas.w, 0)
            , p2.x + p2.radius
          )
        )
      , 1);*/
      ...
      const threshold = 300;
      const pos1X = p1.x + p1.radius;
      const pos2X = p2.x + p2.radius;
      const pos1Y = p1.y + p1.radius;
      const pos2Y = p2.y + p2.radius;

      let opacityLeftEdge = 1;
      let opacityRightEdge = 1;
      let opacityTopEdge = 1;
      let opacityBottomEdge = 1;
      if (pos1X < threshold || pos2X < threshold) {
        opacityLeftEdge = Math.min(pos1X / threshold, pos2X / threshold);
      }
      if (pos1X > pJS.canvas.w - threshold || pos2X > pJS.canvas.w - threshold) {
        opacityRightEdge = Math.min((pJS.canvas.w - pos1X) / threshold, (pJS.canvas.w - pos2X) / threshold);
      }
      if (pos1Y < threshold || pos2Y < threshold) {
        opacityTopEdge = Math.min(pos1Y / threshold, pos2Y / threshold);
      }
      if (pos1Y > pJS.canvas.h - threshold || pos2Y > pJS.canvas.h - threshold) {
        opacityBottomEdge = Math.min((pJS.canvas.h - pos1Y) / threshold, (pJS.canvas.h - pos2Y) / threshold);
      }

      opacity_line = Math.min(Math.min(opacityLeftEdge, opacityRightEdge), Math.min(opacityTopEdge, opacityBottomEdge));
